...
func new_game() -> void:
	...
	save.base_state = {"inventory": {"ore": 6}, "jobs": []}
...
func save_game(slot := "slot1") -> bool:
	...
	save.base_state = {"inventory": base.inventory.duplicate(true), "jobs": _serialize_jobs(base.jobs)}
	var io := SaveIO.new()
...
func _init_runtime_from_save() -> void:
	...
	var raw_species = reg.load_json("res://data/pets/sample_species.json")
	if typeof(raw_species) == TYPE_DICTIONARY:
		var sp := PetSpecies.from_dict(raw_species)
		if sp.id != "":
...
func _deserialize_jobs(raw: Variant) -> Array:
	...
	for e in raw:
		...
		var j := ProductionJob.new(rid, qty, ticks_left, outputs)
